...
00b has no dependency on 01 and can run in a worktree alongside it; both must land before 02/03/04 start. 02, 03 and 04 then run in parallel worktrees. 05 needs the editor's content model. 06 needs the public surfaces. 07 is last and alone.
...
2. **`packages/contracts/` and `specs/shortcuts.md` are read-only in every worktree.** A worktree needing a new key binding stops and asks, exactly as it would for a schema change — this is the collision that parallel work creates and nothing else catches.
3. **`packages/contracts/` is read-only in every worktree.** A contract change means stopping all three sessions, changing it on `main`, and rebasing.
