...
import { validateKey } from '@/lib/ai/validate-key';

export async function saveApiKeyAction(
  provider: string,
  apiKey: string
): Promise<Result<{ savedAt: string; usage?: { used?: number; limit?: number; remaining?: number; resetAt?: string } }>> {
  ...
  return ok({ savedAt: res.value.createdAt, usage: validation.usage });
}
...
export async function deleteApiKeyAction(provider: string): Promise<Result<void>> {
  ...
  const res = await deleteApiKey(user.id, provider as 'openai' | 'anthropic' | 'google');
  return res;
}

export async function refreshApiKeyAction(provider: string): Promise<Result<{ usage?: { used?: number; limit?: number; remaining?: number; resetAt?: string } }>> {
  const user = await getSessionUser();
  if (!user) return err('error.unauthorized');

  // Get the decrypted key to validate
  const { getDecryptedKey } = await import('@/lib/db/ai-keys');
  const keyRes = await getDecryptedKey(user.id, provider as 'openai' | 'anthropic' | 'google');
  if (!keyRes.ok) {
  ...
  const validation = await validateKey(provider as 'openai' | 'anthropic' | 'google', apiKey);

  // Update the stored key with new validation info
  const { saveApiKey } = await import('@/lib/db/ai-keys');
  const updateRes = await saveApiKey(user.id, provider as 'openai' | 'anthropic' | 'google', apiKey, {
    usage: validation.usage,
    isValid: validation.valid,
  });
  ...
  return ok({ usage: validation.usage });
}

export async function getApiKeyStatusAction(): Promise<
  Result<{ hasKeys: Record<'openai' | 'anthropic' | 'google', boolean>; isPro: boolean }>
> {
  const user = await getSessionUser();
  if (!user) return err('error.unauthorized');
  ...
  const hasKeys: Record<'openai' | 'anthropic' | 'google', boolean> = {
    openai: false,
    anthropic: false,
    google: false,
  };
  ...
  for (const k of keysRes.value) {
    hasKeys[k.provider as 'openai' | 'anthropic' | 'google'] = true;
  }
